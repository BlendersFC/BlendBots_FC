...
def main():
    ...
    def get_submode(byte):
        ...
        return subdode.get(byte)
    
    # #dictionary for submode
    # def get_submode_1(byte):
    # 	submode = {
    ...
    UDP_IPR = "192.168.0.2" #hearing all interfaces, must be changed to referee IP
   ...
   # rospy.loginfo(f"escuchando IP{UDP_IP} al puerto {UDP_PORT}")
    ...
    n = 2 #variable para recorrer por cada jugador 0 es player1, 1 es player2...
    ...
    while not rospy.is_shutdown():
        ...
        if(r_var.first_half==1): #los bytes cambian en el cambio de cancha local es 1, vistante 0
            teaminfo = data[24:29]
            playerinfo = data[290+n*6:296+n*6]
        else:
        ...
        sock_return.sendto(packed_data, (UDP_IPR,UDP_PORTR))
        pub.publish(r_var)
